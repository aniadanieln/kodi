...
def CATEGORIES():
        addDir('Nowe','http://www.youjizz.com/newest-clips/1.html',1,'')
        addDir('Najlepsze','http://www.youjizz.com/top-rated/1.html',1,'')
        addDir('Losowe','http://www.youjizz.com/random.php',1,'')
        INDEX('http://www.youjizz.com/page/1.html')


def INDEX(url):
        addDir('Szukaj','http://www.youjizz.com/srch.php?q=',3,'')
        link = getHtml(url)
        ...
        for nexturl in matchpage:
                addDir('Dalej >>','http://www.youjizz.com' + nexturl,1,'')
...
def VIDEOLINKS(url,name):
        ...
        if not match:
                print "Nie znaleziono adresu URL"
        for url in match:
...
def SEARCHVIDEOS(url):
        searchUrl = url
        vq = _get_keyboard(heading="Czego szukasz?")
        # if blank or the user cancelled the keyboard, return
        if (not vq): return False, 0
        # we need to set the title to our query
        title = urllib.quote_plus(vq)
        searchUrl += title
        print "Szukanie linku: " + searchUrl
        INDEX(searchUrl)
