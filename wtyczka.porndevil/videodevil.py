...
class Main:
    ...
    def playVideo(self, videoItem):
        ...
        if self.currentlist.skill.find('nodownload') == -1:
            if addon.getSetting('download') == 'true':
                ...
                self.pDialog.create('PornDevil', __language__(30050), __language__(30051))
                flv_file = self.downloadMovie(url, title)
                self.pDialog.close()
                if flv_file == None:
                    dialog = xbmcgui.Dialog()
                    dialog.ok('PornDevil Info', __language__(30053))
    ...
    def downloadMovie(self, url, title):
        ...
        if download_path == '':
            try:
                download_path = xbmcgui.Dialog().browse(0, __language__(30017), 'pliki', '', False, False)
    ...
    def run(self):
        ...
        try:
            ...
            if enable_debug:
                traceback.print_exc(file = sys.stdout)
            dialog = xbmcgui.Dialog()
            dialog.ok('KURWA B£¥D WTYCZKI !!!', 'KURWA !!! KURWA!!! KURWA!!! \n\nB³¹d z powodu:\n' + str(e))
