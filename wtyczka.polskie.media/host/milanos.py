...
scriptname = "Filmy online www.mrknow.pl - milanos"
...
class milanos:
    ...
    def listsMainMenu(self, table):
        for num, val in table.items():
            self.add('milanos', 'main-menu', val, 'None', 'None', 'None', 'None', 'None', True, False)
    ...
    def listsCategoriesMenu(self):
        ...
        if len(match) > 0:
            ...
            for i in range(len(match)):
                ...
                self.add('milanos', 'categories-menu', match[i][2].strip(), 'None', 'None', url, 'None', 'None', True, False)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItems(self, url):
        ...
        match = re.compile('<div style="width: 22%;" class="fll borderimg">(.*?)<div style="margin: 0 auto;">(.*?)<a href="(.*?)" title="(.*?)"><img src="(.*?)" class="image lazy" data-original="(.*?)" alt="(.*?)" style="width:125px;height:94px;"/></a>', re.DOTALL).findall(readURL)

        print match
        if len(match) > 0:
            for i in range(len(match)):
                #add(self, service, name,               category, title,     iconimage, url, desc, rating, folder = True, isPlayable = True):
                self.add('milanos', 'playSelectedMovie', 'None', match[i][6].decode('iso-8859-2'), match[i][5], match[i][2], 'aaaa', 'None', True, False)
        match1 = re.compile('<a class="paginate" href="(.*?)">(.*?)</a>').findall(readURL)
        #print ("M",match1,len(match1))
        ile = len(match1) - 1
        self.add('milanos', 'categories-menu', 'Następna', 'None', 'None', mainUrl+ '/' + match1[ile][0], 'None', 'None', True, False)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsPage(self, url):
        ...
        if self.getSizeAllItems(url) > 0  and self.getSizeItemsPerPage(url) > 0:
            ...
            for i in range(int(a)):
                ...
                self.add('milanos', 'items-menu', 'None', title, 'None', destUrl, 'None', 'None', True, False)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))        
    ...
    def getMovieLinkFromXML(self, url):
        ...
        readURL = openURL.read()
        openURL.close()
        match = re.compile('<div class="video_object">(.*?)<iframe width="(.*?)" height="(.*?)" src="(.*?)" frameborder="0" allowfullscreen></iframe>(.*?)</div>', re.DOTALL).findall(readURL)
        print "Film"
        print match
        linkVideo = self.up.getVideoLink(match[0][3])
        return linkVideo
