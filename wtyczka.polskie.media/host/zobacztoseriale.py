...
mainUrl = 'http://zobaczto.tv/'
catUrl = 'http://zobaczto.tv/seriale-online/'
...
class zobacztoseriale:
    def __init__(self):
        log.info('Starting zobacztoseriale.pl')
    ...
    def listsMainMenu(self, table):
        for num, val in table.items():
            self.add('zobacztoseriale', 'main-menu', val, 'None', 'None', 'None', 'None', 'None', True, False)
    ...
    def listsCategoriesMenu(self):
        ...
        if len(match1) > 0:
            ...
            for i in range(len(match1)):
                ...
                self.add('zobacztoseriale', 'categories-menu', match1[i][1].strip(), 'None', 'None', catUrl, 'None', 'None', True, False,'1','kat='+match1[i][0])
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsOther(self, url):
            ...
            if len(match1) > 0:
                for i in range(len(match1)):
                        ...
                        self.add('zobacztoseriale', 'playSelectedMovie', 'None', match1[i][1], 'None', match1[i][0], 'aaaa', 'None', False, True)
    ...
    def listsItemsA(self, url):
       for num, val in LETER_TAB.items():
            self.add('zobacztoseriale', 'page-menu', 'None',  val,  'None', mainUrl, 'aaaa', 'None', True, False)
       xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsS(self, url, strona):
        ...
        if len(match1) > 0:
            for i in range(len(match1)):
                ...
                self.add('zobacztoseriale', 'items-menu', 'None', self.cm.html_special_chars(title),  'None', match1[i][0], 'aaaa', 'None', True, False)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsOst(self, url):
        query_data = { 'url': url, 'use_host': False, 'use_cookie': False, 'use_post': True, 'return_data': True }        
        link = self.cm.getURLRequestData(query_data)
        match = re.compile('<!-- LAST EPISODES -->(.*?)	<!-- TOP SERIES -->', re.DOTALL).findall(link)
        match1 = re.compile('<tr>\r\n\t\t\t\t<td><a href="(.*?)">(.*?)</a></td>\r\n\t\t\t\t<td>(.*?)</td>\r\n\t\t\t\t<td>(.*?)</td>\r\n\t\t\t\t<td><a href="(.*?)">(.*?)</a></td>\r\n\t\t\t</tr>', re.DOTALL).findall(match[0])
        if len(match1) > 0:
            for i in range(len(match1)):
                self.add('zobacztoseriale', 'playSelectedMovie', 'None', self.cm.html_special_chars(match1[i][1].strip() + ' S' + match1[i][2].strip() + ' E' + match1[i][3].strip() +' ' + match1[i][5].strip()),  'None', match1[i][4], 'aaaa', 'None', False, True,'')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    def listsItemsTop(self, url):
        ...
        if len(match1) > 0:
            for i in range(len(match1)):
                #print ("M",match1[i])
                self.add('zobacztoseriale', 'playSelectedMovie', 'None', self.cm.html_special_chars(match1[i][1].strip() + ' ' + match1[i][3].strip() + ' ' + match1[i][4].strip()),  'None', mainUrl[:-1]+ match1[i][2], 'aaaa', 'None', False, False,'')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsPage(self, url):
        ...
        if self.getSizeAllItems(url) > 0  and self.getSizeItemsPerPage(url) > 0:
            ...
            for i in range(int(a)):
                ...
                self.add('zobacztoseriale', 'items-menu', 'None', title, 'None', destUrl, 'None', 'None', True, False)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))        
